...
var initMongooseWithSchemaAndModel = function(callbackWithPersistentModel) {
    ...
    db.once('open', function() {
      ...
      if(callbackWithPersistentModel)
           callbackWithPersistentModel(PersistentDeviseModel);
    });
}
  ...
  initMongooseWithSchemaAndModel(
    function(PersistentDeviseModel) {
      ...
      PersistentDeviseModel.deleteMany( deleteAllFilter, function (err) {
        ...
        (new PersistentProduitModel({ code : "USD" , nom : "Dollar" , change : 1.1})).save();
        (new PersistentProduitModel({ code : "GBP" , nom : "Livre" , change : 0.90})).save();
        (new PersistentProduitModel({ code : "JPY" , nom : "Yen" , change : 123})).save();
      })
  });
}
